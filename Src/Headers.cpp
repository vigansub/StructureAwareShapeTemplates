...
#else
void renderString(const char *str, int x, int y, const Vector4 &color, void *font)
#endif
{
...
void saveFrameBuffer(QGLWidget *widget)
{
    ...
    QString fileName = "Output/" + sYear + fMonth + fDay + "_" + fHour + fMinute + fSecond + ".jpg";
...
void saveFrameBuffer(QGLWidget *widget, QString name)
{
    ...
    //QString fileName = "Output/" + sYear + fMonth + fDay + "_" + fHour + fMinute + fSecond + "_" + number + ".jpg";
    QString fileName = "Output/Video/" + name + ".jpg";
    //QString fileName = name + ".jpg";
...
vec3 toTopOffset(const vec3 &size, float mod)
{
    return vec3(0.0, size.y*mod, 0.0);
}

vec3 toBottomOffset(const vec3 &size, float mod)
{
    return vec3(0.0, -size.y*mod, 0.0);
}

vec3 toFrontOffset(const vec3 &size, float mod)
{
    return vec3(0, 0, size.z*mod);
}

vec3 toBackOffset(const vec3 &size, float mod)
{
    return vec3(0, 0, -size.z*mod);
}

vec3 toLeftOffset(const vec3 &size, float mod)
{
    return vec3(-size.x*mod, 0, 0);
}

vec3 toRightOffset(const vec3 &size, float mod)
{
    return vec3(size.x*mod, 0, 0);
}
