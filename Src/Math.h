...
template <typename T> 
T normalDistribution(T mu, T sigma)
{
    if(sigma < math_epsilon)
        return 0.0;
...
template <typename T>
T normalDistributionDerivationMu(T x, T m, T s)
{
    if(s < math_epsilon)
        return 0.0;

    float inv_sqrt_2Pi = 1.0f / (std::sqrt(2*math_pi) * s);

    float e = std::exp(-((x-m)*(x-m)) / (2 * s *s));
    float d = inv_sqrt_2Pi * e;
...
template <typename T>
T normalDistributionDerivationSigma(T x, T m, T s)
{
    if(s < math_epsilon)
        return 0.0;
    ...
    float r = d / s * ( ((x-m)*(x-m)) / (s*s) - 1.0f);
    return r;
}

template <typename T>
T logOfNormalDistributionDerivationMu(T x, T m, T s)
{
    if(s < math_epsilon)
        return 0.0;
...
template <typename T>
T logOfNormalDistributionDerivationSigma(T x, T m, T s)
{
    if(s < math_epsilon)
        return 0.0;

    float r = 1.0 / s * ( (((x-m) * (x-m)) / (s * s) ) - 1.0);
